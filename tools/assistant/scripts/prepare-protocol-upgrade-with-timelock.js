...
async function prepareProtocolUpgradeWithTimelock() {
    try {
        ...
        const timelockAction = await promptUser(
            ...
            ['Queue Transaction', 'Cancel Transaction', 'Execute Transaction']
        );
        ...
        let timelockCalldata;
        let actionName;

        switch (timelockAction) {
            case 0:
                timelockCalldata = timelockInterface.encodeFunctionData('queueTransaction', timelockParams);
                actionName = 'queueTransaction';
                break;
            case 1:
                timelockCalldata = timelockInterface.encodeFunctionData('cancelTransaction', timelockParams);
                actionName = 'cancelTransaction';
                break;
            case 2:
                timelockCalldata = timelockInterface.encodeFunctionData('executeTransaction', timelockParams);
                actionName = 'executeTransaction';
                break;
        }
        ...
        console.log('\nPreparing Rollback Diamond Cut...');
        ...
        console.log('\nRollback Timelock Details:');
        console.log('Action: queueTransaction');
        console.log(`Target: ${targetAddress}`);
        console.log(`ETA: ${new Date(eta * 1000).toISOString()}`);
        console.log('\nRollback Timelock Calldata:');
        console.log(rollbackTimelockCalldata);
